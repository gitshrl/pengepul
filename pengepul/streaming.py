...
from .translate import ResponsesDrain, update_drain_from_response_event, usage_from_responses
...
def anthropic_sse_to_chat(
    ...
    if event == "message_delta":
        ...
        if stop_reason:
            return [_chat_done(state, "length" if stop_reason == "max_tokens" else "stop")]
...
def responses_sse_to_chat(
    ...
    if event in ("response.completed", "response.incomplete"):
        chunks = [_chat_done(state, "length" if event == "response.incomplete" else "stop")]
                ...
                sse(
                    {
                        ...
                        "delta": {
                            "stop_reason": (
                                "max_tokens" if event == "response.incomplete" else "end_turn"
                            ),
                            "stop_sequence": None,
                        },
